...
then
	...
	echo "Example: ./TinyFastSimulator-Balanced-Benchmark.sh 32 100000000 results.txt"
	exit 1
fi
...
do	
	...
	echo -en "$x\t" >> $3
	\time -f "%e\t%M" -o $3 -a java -jar TinyFastSimulator-Java.jar load_balancer threads=$x arrivals=$2 increase_arrivals > /dev/null
done